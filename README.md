...
GitHub stores your code, but it does not keep a FastAPI backend running by itself. To keep JARVIS online like a hosted AI app, connect the GitHub repo to a deployment platform such as Render.

This repo now includes [render.yaml](render.yaml), which deploys:

- the Python FastAPI backend
- the Vite frontend build
- one combined web app from the same repository
...
5. Add your environment variables in Render:
   ...
   - `FISH_API_KEY`
   - `USER_NAME`
   - any other keys you use locally

After deployment, Render gives you an HTTPS URL that keeps the frontend and backend together in one app.

### Cloud Limitations

The hosted version is ideal for chat, voice UI, and browser access, but local desktop-control features do not work in cloud deployment:

- opening apps on your Windows PC
- controlling mouse and keyboard
- reading local desktop windows directly

Those features still work in your local Windows version.
...
Powered by [Anthropic Claude](https://anthropic.com), [OpenRouter](https://openrouter.ai), and [Fish Audio](https://fish.audio).
